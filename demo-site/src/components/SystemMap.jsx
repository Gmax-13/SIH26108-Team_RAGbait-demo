 ...
 *
 *  Components light up from recorded stage events, so what is highlighted is
 *  what the server actually reached on the captured run. The ingestion cluster
 *  pulses on its own timer — it is genuinely always running, independent of any
 *  query, which is the thing that part of the diagram is there to communicate.
 */
...
// [from, to, kind, fromPort, toPort, bend, drop]
  ...
  return (
    <div className="panel sysmap">
      <div className="row split">
        <div>
          <h2>{done ? 'What just happened' : 'System architecture'}</h2>
          <p className="sub">
            Components light up as the pipeline reaches them. Ingestion runs on its
            own schedule, continuously — it is never in the query path.
          </p>
        </div>
        {liveStage && (
